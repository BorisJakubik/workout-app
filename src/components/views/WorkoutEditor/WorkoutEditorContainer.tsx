...
import type { RestTimerState } from './WorkoutEditorView.types'
...
export const WorkoutEditorContainer = ({ draft, exercises, workouts = [], setDraft, finish, cancel, weightUnit }: WorkoutEditorContainerProps) => {
  ...
  const pauseTimer = () => {
    const timerElapsedSeconds = storedElapsedSeconds + Math.max(0, Math.floor((Date.now() - new Date(draft.startedAt).getTime()) / 1000))
    setElapsedSeconds(timerElapsedSeconds)
