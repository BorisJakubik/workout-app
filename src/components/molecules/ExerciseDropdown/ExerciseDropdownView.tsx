    ...
    {open && (
      <div className="exercise-dropdown-menu" role="listbox">
        {visibleExercises.map(exercise => (
          <button
            className={value === exercise.name ? 'selected' : ''}
            type="button"
            role="option"
            aria-selected={value === exercise.name}
            key={exercise.id}
            onClick={() => onSelect(exercise.name)}
          >
            {exercise.name}
            {value === exercise.name && <Check size={15} />}
          </button>
        ))}
        {canShowAll && (
          <button className="expand-exercises" type="button" onClick={onExpand}>
