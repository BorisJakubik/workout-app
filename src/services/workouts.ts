...
const mapWorkout = (row: any): Workout => ({
  id: row.id,
  workoutNumber: Number(row.workout_number),
  ...
  exercises: (row.workout_exercises || [])
    ...
    .map((exercise: any) => ({
      ...
      sets: (exercise.exercise_sets || [])
        .sort((a: any, b: any) => a.position - b.position)
        .map((set: any) => ({ reps: set.reps, weight: Number(set.weight) })),
    })),
})
...
async function replaceExercises(workoutId: string, exercises: WorkoutExercise[]) {
  ...
  const { data: rows, error } = await db
    ...
    .insert(exercises.map((item, position) => ({ workout_id: workoutId, exercise_id: item.exerciseId || null, exercise_name: item.name, position })))
    .select()
  if (error) throw error
  const sets = rows.flatMap((row: any, index: number) =>
    exercises[index].sets.map((set, position) => ({ workout_exercise_id: row.id, reps: set.reps, weight: set.weight, position })),
  )
  if (sets.length) {
    const { error: setsError } = await db.from('exercise_sets').insert(sets)
...
export async function createWorkout(workout: Omit<Workout, 'id'>): Promise<Workout> {
  const db = requireSupabase()
  const { data, error } = await db.from('workouts').insert(payload(workout)).select().single()
  if (error) throw error
