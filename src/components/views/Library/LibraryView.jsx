        ...
        {categories.map(category => {
          ...
          <article className={`library-group${collapsed ? ' collapsed' : ''}`} key={category.id}>
            <div className="library-title">
              <div className="library-category-main"><button className="library-collapse-button" type="button" aria-label={t(collapsed ? 'expandCategory' : 'collapseCategory')} aria-expanded={!collapsed} onClick={() => props.onToggleCategory(category.id)}><ChevronDown size={18} /></button><div className="library-category-icon"><CategoryIcon name={category.icon} size={21} /></div><div>
                <span>{categoryExercises.length} {t('exercises').toUpperCase()}</span>
              ...
              <button aria-label={t('deleteCategory')} onClick={() => props.onRemoveCategory(category)}><Trash2 size={17} /></button>
            </div>
            {!collapsed && <div className="library-group-content"><div className="category-icon-picker">{categoryIconOptions.map(option => <button className={(category.icon || 'bench') === option.id ? 'selected' : ''} key={option.id} aria-label={t(option.labelKey)} title={t(option.labelKey)} onClick={() => props.onIconChange({ id: category.id, icon: option.id })}><option.Icon size={18} /></button>)}</div>
            {categoryExercises.map(exercise => <div className="library-exercise" key={exercise.id}><Dumbbell size={16} /><span>{exercise.name}</span><button aria-label={t('deleteExercise')} onClick={() => props.onRemoveExercise(exercise)}><X size={16} /></button></div>)}</div>}
          </article>
        )})}
      </div>
      {props.deleteItem && <DeleteLibraryItemModalView item={props.deleteItem} onCancel={props.onCancelDelete} onDelete={props.onConfirmDelete} t={t} />}
    </section>
  )
}
