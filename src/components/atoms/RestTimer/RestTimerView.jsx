...
export const RestTimerView = ({ activeDots, formattedTime, isFinished, isRunning, onComplete, onPause, onReset, onResume, t }) => {
  return (
      ...
      <main>
        <div className="rest-timer-ring" role="timer" aria-label={`${t('restTimer')}: ${formattedTime}`}>
          {Array.from({ length: 60 }, (_, index) => (
            <span className={`rest-timer-dot ${index >= 60 - activeDots ? 'active' : ''}`} key={index} style={{ '--angle': `${index * 6}deg` }} />
          ))}
